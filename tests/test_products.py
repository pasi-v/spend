...
def test_insert_and_select_all(conn):
    insert_product(conn, to_slug("milk"), "Whole Milk")
...
def test_insert_normalizes_slug(conn):
    insert_product(conn, to_slug("MILK"), "Whole Milk")
    row = select_product(conn, to_slug("milk"))
...
def test_select_product_not_found(conn):
    assert select_product(conn, to_slug("nope")) is None
...
def test_update_product(conn):
    ...
    assert producer is not None
    insert_product(conn, to_slug("milk"), "Whole Milk")
    update_product(conn, to_slug("milk"), "Skim Milk", producer["producer_id"])
    row = select_product(conn, to_slug("milk"))
    assert row is not None
...
def test_delete_product(conn):
    insert_product(conn, to_slug("milk"), "Whole Milk")
    delete_product(conn, to_slug("milk"))
    assert select_product(conn, to_slug("milk")) is None


def test_require_product_found(conn):
    insert_product(conn, to_slug("milk"), "Whole Milk")
    row = require_product(conn, to_slug("milk"))
    assert row["product_slug"] == "milk"
